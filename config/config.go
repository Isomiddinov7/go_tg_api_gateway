...
func Load() Config {
	...
	config.Domain = cast.ToString(getOrReturnDefaultValue("DOMAIN", "localhost:8081"))

	config.DefaultOffset = cast.ToString(getOrReturnDefaultValue("DEFAULT_OFFSET", "0"))
	config.DefaultLimit = cast.ToString(getOrReturnDefaultValue("DEFAULT_LIMIT", "10"))

	config.CoinServiceHost = cast.ToString(getOrReturnDefaultValue("COIN_SERVICE_HOST", "localhost"))
	config.CoinGRPCPort = cast.ToString(getOrReturnDefaultValue("COIN_GRPC_PORT", ":9101"))
